...
import { getBalance, getMovements, getContactById } from "./store.js";
import { formatMoney, bindNavigationLinks, formatDate } from "./util.js";
    ...
    $movementsList.html(
        movements
            .map(({ tag, registro }) => {
                ...
                return `
                    <li class="list-group-item d-flex justify-content-between align-items-center movement-item">
                        <div>
...
const openModal = (movement) => {
    const isDeposit = movement.tag === "deposit";
    const { registro } = movement;

    $("#modalType").text(isDeposit ? "Depósito" : "Envío de dinero");
    $("#modalAmount").text(`${isDeposit ? "+" : "-"} ${formatMoney(registro.amount)}`);
    $("#modalDate").text(new Date(registro.date).toLocaleString("es-CL"));
    $("#modalId").text(registro.id);

    if (isDeposit) {
        $("#modalContactContainer").addClass("d-none");
    } else {
        const contact = getContactById(registro.idContact);

        $("#modalContact").text(
            contact?.nombre ?? "Contacto no encontrado"
        );

        $("#modalContactContainer").removeClass("d-none");
    }
...
$("#movementsList").on("click", ".movement-item", function () {
    const index = $(this).index();
    const { data } = getMovements(currentPage);
